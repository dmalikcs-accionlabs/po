			...
			helpers.extend(rectangle, {
				...
				draw: function () {
					...
					var halfHeight = vm.height / 2,
						...
						right = vm.skeleton - (vm.skeleton - vm.x),
				...
				inRange: function (mouseX, mouseY) {
					...
					if (vm) {
						if (vm.x < vm.skeleton) {
							inRange = (mouseY >= vm.y - vm.height / 2 && mouseY <= vm.y + vm.height / 2) && (mouseX >= vm.x && mouseX <= vm.skeleton);
						} else {
							inRange = (mouseY >= vm.y - vm.height / 2 && mouseY <= vm.y + vm.height / 2) && (mouseX >= vm.skeleton && mouseX <= vm.x);
						}
					}
	...
	Chart.elements.Rectangle = Chart.Element.extend({
		draw: function() {
			...
			var halfWidth = vm.width / 2,
				...
				top = vm.skeleton - (vm.skeleton - vm.y),
				halfStroke = vm.borderWidth / 2;
		...
		height: function() {
			var vm = this._view;
			return vm.skeleton - vm.y;
		},
		inRange: function(mouseX, mouseY) {
			var vm = this._view;
			return vm ? 
					(vm.y < vm.skeleton ?
						(mouseX >= vm.x - vm.width / 2 && mouseX <= vm.x + vm.width / 2) && (mouseY >= vm.y && mouseY <= vm.skeleton) :
						(mouseX >= vm.x - vm.width / 2 && mouseX <= vm.x + vm.width / 2) && (mouseY >= vm.skeleton && mouseY <= vm.y)) :
					false;
		},
		inLabelRange: function(mouseX) {
			var vm = this._view;
